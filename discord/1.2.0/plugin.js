    ...
    function dispatchEvent(evt, d) {
      switch (evt) {
        ...
        case "VOICE_STATE_CREATE": {
          const uid = d.user && d.user.id ? String(d.user.id) : "";
          const isJoin = uid && !members[uid] && uid !== selfId;
          upsertVoiceState(d);
          if (isJoin && ctx.settings.get("notifyJoin") === true) {
            const m = members[uid];
            const who = (m && m.name) || uid;
            ctx.toast("notice", "Discord", who + " в " + (channelName || "голосовом канале"));
          }
          publish();
          return;
        }
