  ...
  activate(ctx) {
    ...
    function declare() {
      ...
      if (mode !== "meters") list.push({
        id: "docker", label: "Docker · статус", type: "status",
    ...
    function renderStatus(running, total, up) {
      ...
      // число запущенных контейнеров — на шину значений (условия / HA / gauge).
      // Только когда демон доступен: при «down» счётчик неизвестен, не выдумываем 0.
      if (up) p.number = running;
