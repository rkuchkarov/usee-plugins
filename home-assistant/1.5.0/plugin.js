...
definePlugin({
  ...
  activate(ctx) {
    ...
    // Сущности, чьи состояния нужны: ОБЪЕДИНЕНИЕ двух потребителей.
    //
    //  · биндинги карточек — старый путь, пока карточки объявляются здесь;
    //  · спрос агента (`ctx.entities.demanded()`) — привязки конфига: слоты
    //    своих карточек, условия показа и триггеры сцен.
    //
    // Второе появилось вместе с развязкой и раньше не существовало вовсе:
    // подписка шла только по СВОЕМУ списку настроек, из-за чего на живом
    // конфиге было подписано девять сущностей при пяти, на которые
    // действительно ссылались привязки.
    //
    // Объявление (declare) сюда НЕ входит: объявлять можно всё, что знает
    // сервер, — это строка в каталоге и ничего не стоит. Подписка стоит
    // трафика, и потому идёт только по потребителям.
    function wanted() {
      const set = {};
      ...
      for (const id of ctx.entities.demanded()) set[String(id).toLowerCase()] = true;
    ...
    function handle(s, msg, access) {
      switch (msg.type) {
        ...
        case "event": {
          ...
          if (to) { storeState(to); publish(); publishEntities(); }
          return;
        }
      }
    }

    // Подписка на изменения состояний. Набор задаётся СПРОСОМ (плюс биндинги
    // карточек), а он меняется от привязки в конфиге — значит подписку надо
    // уметь пересобирать, не разрывая соединение.
    //
    // Старая подписка снимается ЯВНО (`unsubscribe_events`): без этого сервер
    // продолжал бы слать события по прежнему набору, и после нескольких правок
    // мы получали бы по нескольку копий каждого изменения.
    ...
    function subscribeStates(s) {
      ...
      if (triggerSubId > 0) {
        try { s.send(JSON.stringify({ id: ++cmdId, type: "unsubscribe_events", subscription: triggerSubId })); }
        catch { /* сокет уже закрыт — переподключение всё пересоберёт */ }
        triggerSubId = 0;
      }
    ...
    // Спрос изменился на живом соединении.
    function resubscribe() {
      if (!sock) return;                     // не подключены — пересоберём на auth_ok
      try { subscribeStates(sock); }
      catch (e) { ctx.log.warn("пере-подписка: " + e); }
    }
    ...
    // ---- сущности: объявляем ВСЁ, что знает сервер ----------------------
    //
    // Каталог сервера шире шины: у пользователя 573 сущности против девяти,
    // ради которых заводились карточки. Объявляются все — тогда в пикере видно
    // «найдено», а дробь «публикуется из известно» показывает разницу без
    // единого слова. Значения гонятся только по спросу.

    // Тип определяется по домену и по текущему состоянию, и только ЧЕСТНО:
    // домен switch/binary_sensor даёт булево, разбираемое число — число, всё
    // остальное — строку. Гадать нельзя: объявленный числом текстовый сенсор
    // молча не привяжется к числовому слоту.
    function entityKind(id, st) {
      if (isBoolDomain(id)) return "bool";
      if (st && honestNumber(st.state) !== null) return "number";
      return "string";
    }
    ...
    function declareEntities() {
      ...
      for (const id of Object.keys(states)) {
        ...
        out.push({
          id: id,
          label: (st && st.friendly) || id,
          kind: entityKind(id, st),
          unit: (st && st.unit) || undefined,
          // Раздел карты — ДОМЕН, он же начало адреса: человек его и так
      ...
      ctx.log.info("сущностей объявлено: " + out.length);
    }

    // Значения — только спрошенных. Несуществующие у сервера пропускаются
    // молча: спрос мог остаться от сущности, которую в Home Assistant удалили,
    // и ругаться на это в лог каждый такт бесполезно.
    function publishEntities() {
      const want = ctx.entities.demanded();
      ...
      for (const raw of want) {
        ...
        const st = states[id];
        if (!st) continue;
        if (isUnavailable(st.state)) { vals[raw] = null; continue; }
        const kind = entityKind(id, st);
        vals[raw] = kind === "bool" ? boolOf(id, st.state)
                  : kind === "number" ? honestNumber(st.state)
                  : st.state;
      }
      ctx.entities.setMany(vals);
    }
    ...
    // Спрос изменился — привязали или сняли адрес. Подписка обязана появиться
    // в момент привязки, иначе карточка откроется пустой до первого значения;
    // поэтому сперва отдаём то, что уже знаем, и только потом просим сервер
    // присылать обновления по новому набору.
    ctx.entities.onDemandChanged(() => {
      publishEntities();
      resubscribe();
    });
