...
definePlugin({
  activate(ctx) {
    ...
    function fieldPath(e) {
      return e.address.length > e.cardKey.length ? e.address.slice(e.cardKey.length + 1) : "";
    }
    const TRIAD = ["number", "is_on", "state"];   // порядок приоритета

    // Default-набор карточки целиком: ОДНО привилегированное поле триады —
    // number → is_on → state по убыванию наличия (у сущности практически
    // никогда не бывает двух сразу) — плюс кастомные поля с ha-метой (напр.
    // discord any(speaking)/_present). Автополя без меты в набор не входят.
    function defaultSetOf(cardKey, cat) {
      const triad = {};
      const custom = [];
      for (const e of cat) {
        if (e.cardKey !== cardKey) continue;
        const p = fieldPath(e);
        if (TRIAD.indexOf(p) >= 0) triad[p] = e;
        else if (hasMeta(e.ha)) custom.push(e);
      }
      const out = [];
      for (const t of TRIAD) if (triad[t]) { out.push(triad[t]); break; }   // привилегированное
      return out.concat(custom);
    }

    // выбранные источники → { object_id → каталожная запись }; коллизии
    // object_id пропускаются с внятной ошибкой в лог.
    function desiredEntities() {
      ...
      for (const s of sources) {
        ...
        for (const e of defaultSetOf(s, cat)) wanted.push(e);         // карточка целиком
      }
      const out = {};
    ...
    function fmtNumber(e, num) {
      const u = e.ha && e.ha.unit ? String(e.ha.unit).trim().toLowerCase() : "";
