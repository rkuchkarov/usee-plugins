...
definePlugin({
  activate(ctx) {
    ...
    const TOASTS_ADDRESS = "sys.toasts";

    function snake(address) {
      return address.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
    }

    // Выбранное → { object_id → каталожная запись }; коллизии object_id
    // пропускаются с внятной ошибкой в лог.
    //
    // РАНЬШЕ строка настройки имела ДВА смысла: адрес поля либо ключ карточки,
    // и второй разворачивался в «набор по умолчанию» — одно привилегированное
    // поле триады (number → is_on → state) плюс поля с ha-метой. Ухищрение
    // существовало потому, что карточка разворачивалась в несколько адресов, из
    // которых настоящим значением было одно.
    //
    // После развязки одна сущность — одно значение, разворачивать нечего:
    // настройка хранит ровно адреса. Строка, которой нет в каталоге, — не
    // «карточка целиком», а исчезнувшая сущность, и о ней говорится вслух.
    function desiredEntities() {
      ...
      for (const s of sources) {
        if (byAddress[s]) wanted.push(byAddress[s]);
        else ctx.log.warn("источник «" + s + "» больше не существует — пропущен");
      }
      const out = {};
    ...
    // Единицы, где дробная часть шума не несёт — округляем до целого; иначе
    // публикуем с точностью до 2 знаков (0.8 не превращаем в 1).
    const COARSE_UNITS = { "%": 1, "°c": 1, "c": 1, "w": 1, "min": 1, "мин": 1 };
    function fmtNumber(e, num) {
      const u = e.unit ? String(e.unit).trim().toLowerCase() : "";
      if (COARSE_UNITS[u]) return String(Math.round(num));
      return String(Math.round(num * 100) / 100);   // до 2 знаков, без хвостовых нулей
    }
    function stateOf(e, v) {
      if (v.state !== "ok") return null;
      if (e.kind === "bool") return v.value ? "ON" : "OFF";
      if (e.kind === "number") return fmtNumber(e, Number(v.value));
    ...
    // тосты → event-сущность немедленно (если выбрана в дереве). object_id
    // считается тем же snake() от адреса, что и у остальных: имя, записанное
    // здесь отдельной строкой, разъехалось бы с адресом при первом же его
    // изменении — и разъехалось, когда встроенные сущности переехали под `sys`.
    ctx.data.toasts.subscribe(t => {
      const obj = snake(TOASTS_ADDRESS);
