...
Показывает на дисплее лимиты Claude Code одной карточкой `meters`:

- бар **«Сессия»** — 5-часовая сессия, бар **«Неделя»** — недельный лимит (какие
  строки показывать — настройка «Строки»);
- футер **«СБРОС …»** — время до сброса 5-часовой сессии («2:14»), а если недельный
  лимит дошёл до порога («Показывать сброс недели при заполнении ≥ N%», по
  умолчанию 100) — до сброса недельного («2д 5ч»).

Ширина карточки — настройки карточки Usee (по умолчанию плагин просит «растягивать»
от 3 полуклеток). SDK 8: опрос файла раз в 10–300 с (по умолчанию 30).

Процент сессии уходит и на шину значений (`number`) — для условий сцен / экспорта в HA.
