...
// одной карточкой meters: бары СЕС/НЕД + футер «↻ 2ч 22мин (21:40)» — глиф сброса,
// сколько осталось и абсолютное время сброса (про 5ч-сессию, а если недельная
// заполнена — про неё).
//
...
const RESET_GLYPH = "↻";   // ↻ — глиф сброса, есть в MonoMicro (FONT_MICRO)

function pad2(n) { return (n < 10 ? "0" : "") + n; }

// абсолютное время сброса «(21:40)» из epoch-секунд (локальное время агента)
function clockAt(epoch) {
  const d = new Date(epoch * 1000);
  return "(" + pad2(d.getHours()) + ":" + pad2(d.getMinutes()) + ")";
}
// сессия (5ч-окно): «2ч 22мин» / «22мин» до сброса
function fmtHMru(sec) {
  sec = Math.max(0, Math.round(sec));
  const h = Math.floor(sec / 3600), m = Math.floor((sec % 3600) / 60);
  return h ? (h + "ч " + pad2(m) + "мин") : (m + "мин");
}
...
definePlugin({
  activate(ctx) {
    ...
    function apply(data) {
      ...
      // футер: «↻ 2ч 22мин (21:40)» — глиф сброса + сколько осталось + абсолютное время
      const mk = (epoch, dur) => RESET_GLYPH + " " + dur + " " + clockAt(epoch);
      let footer = "";
      if (m === "week")                            footer = wReset ? mk(wReset, fmtDH(wReset - now)) : "";
      else if (weekFull && wReset && m === "both") footer = mk(wReset, fmtDH(wReset - now));   // упёрлись в недельный
      else if (sReset && m !== "week")             footer = mk(sReset, fmtHMru(sReset - now)); // ждём 5ч-сброс
      else if (wReset)                             footer = mk(wReset, fmtDH(wReset - now));
