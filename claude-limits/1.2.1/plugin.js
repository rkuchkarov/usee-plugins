...
// недельная заполнена — про неё). Компактный формат — длинная строка не влезает в
// узкий футер meters-карточки.
//
...
definePlugin({
  activate(ctx) {
    ...
    // Порог недельного заполнения, с которого футер показывает сброс НЕДЕЛИ, а не
    // 5ч-сессии (по умолчанию 100 % — как раньше, переключение лишь при упоре).
    function weekFooterAt() {
      const n = Number(ctx.settings.get("weekFooterAt"));
      return isFinite(n) && n >= 0 && n <= 100 ? n : 100;
    }
    ...
    // Одна meters-карточка: бары сессии/недели + футер «СБРОС …». Футер про 5ч-сессию,
    ...
    function apply(data) {
      ...
      // label — короткая форма для узких карточек, label16 — для 264 px.
      // Лейбл карточки не задаём: он опционален и стоит СТРОКИ на 129×109, а
      // «CLAUDE» над «Сессия / Неделя» дублировал бы контекст, который уже несёт
      // широкая форма строки («Claude · сессия»).
      const rows = [];
      if (m !== "week" && sPct !== null)
        rows.push({ label: "Сессия", label16: "Claude · сессия", pct: sPct });
      if (m !== "session" && wPct !== null)
        rows.push({ label: "Неделя", label16: "Claude · неделя", pct: wPct });
      ...
      else if (weekHot && wReset && m === "both") footer = "СБРОС " + fmtDH(wReset - now);   // неделя у порога → её сброс важнее
      ...
      ctx.cards.upsert("limits", {
        type: "meters", rows: rows, footer: footer,
